...
int main(int argc, char *argv[]) {
	...
	void *data_d, *scores_d, *boxes_d, *landms_d;
	...
	cudaMalloc(&landms_d, num_det * 10 * sizeof(float));
	...
	auto start = chrono::steady_clock::now();
 	vector<void *> buffers = { data_d, scores_d, boxes_d, landms_d };
	...
	unique_ptr<float[]> landms(new float[num_det * 10]);
	...
	cudaMemcpy(landms.get(), landms_d, sizeof(float) * num_det * 10, cudaMemcpyDeviceToHost);
